...
class ValidatorAgent:
    ...
    def _systems_engineering_check(self, mission: str, text: str) -> List[ValidationFinding]:
        findings = []
        required_terms = {
            ...
            "verification": ["verify", "verification", "inspection", "analysis", "demonstration", "test"],
            "validation": ["validate", "validation", "intended use", "operational environment"],
            "risk": ["risk", "failure", "hazard", "mitigation", "safety"],
            "approval gates": ["approval gate", "human approval", "needs_review", "Henry must approve"],
        }
    ...
    def _physics_check(self, text: str) -> List[ValidationFinding]:
        ...
        unit_patterns = [
            r"\bkg\b", r"\bg\b", r"\bN\b", r"\bNm\b", r"\bN·m\b",
            r"\bm/s\b", r"\bm/s\^2\b", r"\bW\b", r"\bJ\b",
            r"\bV\b", r"\bA\b", r"\bmA\b", r"\bOhm\b", r"\bΩ\b",
            r"\bmm\b", r"\bcm\b", r"\bm\b", r"\bdeg\b", r"\bdegree",
        ]
        ...
        physics_terms = ["force", "mass", "weight", "torque", "energy", "power", "velocity", "acceleration", "moment of inertia"]
    ...
    def _mechanical_check(self, text: str) -> List[ValidationFinding]:
        ...
        mechanical_terms = [
            "stress", "strain", "bending", "torsion", "stiffness",
            "yield", "fracture", "fatigue", "load path", "factor of safety",
            "clearance", "mounting", "vibration", "backlash", "servo overload",
        ]
        ...
        if self._contains_any(text, ["material", "aluminum", "carbon fiber", "steel", "pla", "petg", "abs", "acrylic"]):
            findings.append(self._pass(
                "Mechanical",
        ...
        if self._contains_any(text, ["mounting hole", "standoff", "screw", "m3", "clearance", "cutout"]):
            findings.append(self._pass(
                "Mechanical",
    ...
    def _electrical_check(self, text: str) -> List[ValidationFinding]:
        findings = []
        electrical_terms = [
            "battery", "voltage", "current", "amp", "watt", "power",
            "esc", "motor", "servo", "resistor", "capacitor", "wire", "connector",
            "gpio", "i2c", "spi", "uart", "pwm", "regulator",
        ]
        ...
        has_voltage = bool(re.search(r"\b\d+(\.\d+)?\s?V\b", text, flags=re.IGNORECASE)) or self._contains_any(text, ["voltage"])
        has_current = bool(re.search(r"\b\d+(\.\d+)?\s?(A|mA)\b", text, flags=re.IGNORECASE)) or self._contains_any(text, ["current draw", "current"])
        has_power_risk = self._contains_any(text, ["power supply", "voltage drop", "overcurrent", "short", "thermal", "heat"])
    ...
    def _controls_check(self, text: str) -> List[ValidationFinding]:
        ...
        controls_terms = [
            "feedback", "controller", "pid", "lqr", "stability",
            "sensor", "actuator", "disturbance", "noise", "latency",
            "closed loop", "open loop", "overshoot", "settling",
        ]
        ...
        if self._contains_any(text, ["stability", "unstable", "oscillation", "overshoot", "settling", "backlash"]):
            findings.append(self._pass(
                "Controls",
    ...
    def _robotics_dynamics_check(self, mission: str, text: str) -> List[ValidationFinding]:
        findings = []

        robotics_terms = [
            "dynamics", "state", "input", "constraint", "trajectory",
            "actuator limit", "sensor", "servo", "robot", "ros2", "node",
        ...
        drone_mission = any(word in mission_lower or word in text_lower for word in ["drone", "quadcopter", "hexacopter", "multirotor", "uav"])

        if drone_mission:
            if self._contains_any(text, ["thrust-to-weight", "thrust to weight", "payload", "total thrust", "hover throttle"]):
                findings.append(self._pass(
                    "Drone Feasibility",
    ...
    def _estimation_check(self, text: str) -> List[ValidationFinding]:
        ...
        estimation_terms = [
            "kalman", "bayesian", "filter", "sensor fusion",
            "imu", "gps", "camera", "lidar", "state estimation",
            "measurement noise", "process noise", "ultrasonic",
        ]
        ...
        if self._contains_any(text, ["measurement noise", "process noise", "calibration", "drift", "latency"]):
            findings.append(self._pass(
                "Estimation / Sensor Fusion",
    ...
    def _ros2_check(self, mission: str, text: str) -> List[ValidationFinding]:
        ...
        ros2_terms = ["node", "topic", "message_type", "launch", "package", "publisher", "subscriber"]
        ...
        if self._contains_any(text, ["ros2 launch", "ros2 topic", "ros2 node", "launch.py", "package.xml", "cmakelists"]):
            findings.append(self._pass(
                "ROS2 Architecture",
    ...
    def _cad_check(self, mission: str, text: str) -> List[ValidationFinding]:
        ...
        mission_mentions_cad = any(term in mission.lower() or term in text.lower() for term in ["fusion 360", "cad", "enclosure", "blueprint", "3d model"])

        if not mission_mentions_cad:
            return findings

        if self._contains_any(text, ["fusion 360", "cad", "sketch", "extrude", "fillet", "component", "assembly"]):
            findings.append(self._pass(
                "CAD / Fusion 360",
        ...
        if self._contains_any(text, ["mm", "mounting", "clearance", "standoff", "screw", "cutout", "manufacturing"]):
            findings.append(self._pass(
                "CAD / Fusion 360",
    ...
    def _objective_evidence_check(self, text: str) -> List[ValidationFinding]:
        ...
        evidence_terms = [
            "test result", "logged result", "measured", "measurement",
            "datasheet", "benchmark", "simulation result", "prototype test",
            "verified with", "validated by", "observed",
        ]
        ...
        placeholder_terms = [
            "unknown", "must be checked", "needs to measure", "still needs",
            "estimate", "assumption", "assumed", "to be confirmed",
            "should be tested", "pending",
        ]
    ...
    def _safety_gate_check(self, text: str) -> List[ValidationFinding]:
        ...
        if self._contains_any(text, ["stop condition", "safety gate", "do not", "must not", "unsafe", "needs_review"]):
            findings.append(self._pass(
                "Safety Gates",
    ...
    def _score_by_category(self, findings: List[ValidationFinding]) -> Dict[str, float]:
        value = {"PASS": 9.0, "WARNING": 5.5, "FAIL": 1.5}
    ...
    def _overall_score(self, scores: Dict[str, float], findings: List[ValidationFinding]) -> float:
        if not scores:
            return 0.0
        ...
        # Very important: prevent concept-stage reports from receiving perfect scores
        # while they still contain warnings.
        if warning_count > 0:
            score = min(score, 8.6)
    ...
    def _verdict_from_score(self, score: float, findings: List[ValidationFinding], text: str) -> str:
        fail_count = sum(1 for f in findings if f.status == "FAIL")
        warning_count = sum(1 for f in findings if f.status == "WARNING")

        has_objective_evidence_warning = any(
            f.category == "Objective Evidence" and f.status in {"WARNING", "FAIL"}
            for f in findings
        )

        has_pending_unknowns = self._contains_any(
            text,
            ["unknown", "must be checked", "needs to measure", "still needs", "pending", "to be confirmed"],
        )
    ...
    def _collect_missing_evidence(self, findings: List[ValidationFinding]) -> List[str]:
        missing = []
        for finding in findings:
    ...
    def _recommend_next_tests(self, findings: List[ValidationFinding], artifact_type: str) -> List[str]:
        tests = []
        categories_with_failures = {f.category for f in findings if f.status == "FAIL"}
        categories_with_warnings = {f.category for f in findings if f.status == "WARNING"}

        if "Systems Engineering" in categories_with_failures:
            tests.append("Create a requirements-verification-validation matrix before continuing.")

        if "Physics" in categories_with_failures or "Physics" in categories_with_warnings:
            tests.append("Run a unit and dimensional-analysis pass on every numerical claim.")

        if "Mechanical" in categories_with_failures or "Mechanical" in categories_with_warnings:
            tests.append("Measure component mass, estimate load path, and verify mounting/clearance dimensions.")

        if "Electrical / Power" in categories_with_failures or "Electrical / Power" in categories_with_warnings:
            tests.append("Create a power budget with voltage, estimated current draw per component, total current, and protection method.")

        if "Controls" in categories_with_failures or "Controls" in categories_with_warnings:
            tests.append("Define the closed-loop control architecture and run a servo sweep/stability test.")

        if "Robotics Dynamics" in categories_with_failures or "Drone Feasibility" in categories_with_failures:
            tests.append("Calculate mass, thrust-to-weight ratio, center of mass, moment of inertia, and actuator limits.")

        if "Planning / Autonomy" in categories_with_failures or "Planning / Autonomy" in categories_with_warnings:
            tests.append("Define state space, action space, initial state, goal state, constraints, and feasibility criterion.")

        if "Estimation / Sensor Fusion" in categories_with_failures or "Estimation / Sensor Fusion" in categories_with_warnings:
            tests.append("Define sensor suite, state vector, prediction model, measurement model, calibration method, and noise assumptions.")

        if "ROS2 Architecture" in categories_with_failures or "ROS2 Architecture" in categories_with_warnings:
            tests.append("Generate ROS2 package files, launch file, node tests, and topic echo verification commands.")

        if "CAD / Fusion 360" in categories_with_failures or "CAD / Fusion 360" in categories_with_warnings:
            tests.append("Create Fusion 360 parameters, mounting dimensions, material assumptions, and an enclosure clearance checklist.")

        if "Objective Evidence" in categories_with_warnings:
            tests.append("Collect objective evidence: measurements, datasheets, simulation logs, ROS2 bag output, or prototype test results.")

        if "Safety Gates" in categories_with_failures:
            tests.append("Define stop conditions before running any hardware or movement test.")

        if not tests:
            tests.append("Proceed to simulation or prototype test with logged results as objective evidence.")
    ...
    def _sanitize_legacy_names(self, text: str) -> str:
    ...
    def _pass(self, category: str, message: str, recommendation: str, source: str) -> ValidationFinding:
        return ValidationFinding(category, "PASS", message, recommendation, source)

    def _warning(self, category: str, message: str, recommendation: str, source: str) -> ValidationFinding:
        return ValidationFinding(category, "WARNING", message, recommendation, source)

    def _fail(self, category: str, message: str, recommendation: str, source: str) -> ValidationFinding:
        return ValidationFinding(category, "FAIL", message, recommendation, source)