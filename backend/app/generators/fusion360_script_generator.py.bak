...
def infer_project_type(mission_result: Dict[str, Any]) -> str:
    ...
    if any(
        word in combined
        for word in ["rover", "mobile robot", "wheel", "wheeled", "tracked", "chassis"]
    ):
        return "rover"

    if any(
        word in combined
        for word in ["robot arm", "manipulator", "joint", "end effector", "end-effector"]
    ):
        return "robot_arm"

    if any(
        word in combined
        for word in ["enclosure", "case", "housing", "box", "electronics box"]
    ):
        return "enclosure"
...
def infer_rotor_count(mission_result: Dict[str, Any]) -> int:
    combined = mission_text(mission_result)
    ...
    if any(
        word in combined
        for word in ["tricopter", "3 motor", "3 motors", "three motor", "three motors", "3 rotor"]
    ):
        return 3
