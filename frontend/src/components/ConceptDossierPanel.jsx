...
function CdChip({ label, variant }) {
  return (
    <span className={`cd-chip cd-chip-${variant}`}>{label}</span>
  );
}
...
function PanelStatusChip({ status }) {
  const variant =
    ...
    "unknown";
  return <span className={`cd-panel-status cd-panel-status-${variant}`}>{status}</span>;
}
...
function DossierPanelCard({ panel }) {
  ...
  const summary = panel.summary && typeof panel.summary === "object" ? panel.summary : null;

  return (
    <div className={`cd-dossier-card cd-dossier-card-${panel.status || "unknown"}`}>
      <div className="cd-dossier-card-header">
        <span className="cd-dossier-card-title">{panel.title}</span>
        <PanelStatusChip status={panel.status} />
      </div>

      {panel.note && (
        <p className="cd-dossier-note">{panel.note}</p>
      )}
      ...
      {summary && (
        <div className="cd-brain-summary">
          {summary.selected_check_count != null && (
            <CdItemRow label="Selected checks" value={String(summary.selected_check_count)} />
          )}
          {summary.readiness_overall_status && (
            <CdItemRow label="Input readiness" value={summary.readiness_overall_status} />
          )}
          {summary.computed_metric_count != null && (
            <CdItemRow label="Computed metrics" value={String(summary.computed_metric_count)} />
          )}
          {summary.blocked_calculation_count != null && (
            <CdItemRow label="Blocked calculations" value={String(summary.blocked_calculation_count)} />
          )}
        </div>
      )}
      ...
      {hero && (
        <CdSection label="Visual preview contract">
          <div className="cd-hero-row">
            <CdItemRow label="Type" value={hero.type?.replace(/_/g, " ") ?? "—"} />
            <CdItemRow label="Label" value={hero.label ?? "—"} />
            {hero.path && <CdItemRow label="Path" value={hero.path} />}
            ...
            {hero.not_cad_accurate && (
              <CdItemRow label="CAD accuracy" value="Concept placeholder only. Not CAD-accurate." />
            )}
          </div>
        </CdSection>
      )}
      ...
      {panels.length > 0 && (
        <CdSection label="Dossier panels">
          <div className="cd-grid">
            {panels.map((panel) => (
              <DossierPanelCard key={panel.id} panel={panel} />
            ))}
          </div>
        </CdSection>
      )}
        ...
        <CdSection label="Source reports">
          <ul className="cd-source-list">
            {sourceReports.map(([key, file]) => (
              <li key={key}>
                <span className="cd-source-key">{key.replace(/_/g, " ")}</span>
